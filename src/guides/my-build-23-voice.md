...
Voice is the convenience layer over everything the earlier pages built — the leak sensors, the locks, the Lutron lights and the SmartWings shades, the scripts and scenes. There are two honest ways to talk to this house, and this build runs both. The **Apple path** uses the iPhones, iPads, and Apple Watches as the microphone, with Siri saying the words and your scripts and scenes as the targets — no extra software, no subscription, and it rides the Tailscale link from the remote-access work so it answers home or away. Add a **HomePod** later and the same Siri path extends to talking to the room hands-free — but nothing here needs an Apple hub to work. The **local path** is Home Assistant's own **Assist**: a Voice Preview Edition puck on the counter, speech-to-text and a conversation model running on the **EVGA GTX 1080 Ti** that was shared into the host earlier, and nothing spoken ever leaving the LAN (local area network). This page builds both.
...
### The HomePod you add later does double duty
The Thread border router on this build is **Home Assistant's own OTBR** (the second ZBT-2 from the Matter Locks page), so a HomePod is never required. But when you add one it earns two jobs at once: "Hey Siri" spoken to the room triggers the scenes and scripts you exposed above — voice with no phone in hand — and the speaker is also the Apple **home hub** for HomeKit away-access. As a bonus it **joins the Thread mesh as a second border router**, strengthening it for the locks and battery shades. Nothing about the phone setup changes; you have just added ears to the house.

> [!DETAILS] The honest limits of the Apple side
> Three things to know. A single bridge tops out at **150 accessories** — generous for scripts and scenes, but real if you ever expose every light and sensor (run a second bridge if needed). **HomeKit Secure Video is not supported**, so this is not the path for the doorbell or any of the cameras — keep watching those through Frigate in Home Assistant. And HomeKit's own away-from-home control is what needs the home-hub HomePod; the phone-and-Shortcuts path never did, because it rides your own Tailscale link instead of Apple's.
...
> [!WARNING]
> Three honest limits on the local side. The puck hardware is still **"preview"** — capable but unfinished. **Music and general knowledge are still better elsewhere** — the HomePod mini will out-answer a local LLM on trivia and out-play it on streaming. And the GPU is shared, so a very heavy Frigate detection moment and a long voice query compete for the same card. The graceful move: if you add the HomePod mini later, keep it for "play something" and the weather, and point local Assist at the house it controls so completely — the locks, the leak valve, the lights, the shades.
