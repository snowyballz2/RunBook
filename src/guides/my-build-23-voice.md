...
> Three things to know. A single bridge tops out at **150 accessories** — generous for scripts and scenes, but real if you ever expose every light and sensor (run a second bridge if needed). **HomeKit Secure Video is not supported**, so this is not the path for the doorbell or the EmpireTech cameras — keep watching those through Frigate in Home Assistant. And HomeKit's own away-from-home control is what needs the home-hub HomePod; the phone-and-Shortcuts path never did, because it rides your own Tailscale link instead of Apple's.
