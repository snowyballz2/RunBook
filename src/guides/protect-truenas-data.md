...
The easiest route is from a Windows machine: TrueNAS automatically presents a share's ZFS snapshots to SMB (Server Message Block) clients as shadow copies, so Windows' standard Previous Versions feature sees them — right-click a file or folder on the share, choose **Properties → Previous Versions**, pick a version, and **Open** or **Restore**. Snapshots are read-only, so no client can alter or delete them.
