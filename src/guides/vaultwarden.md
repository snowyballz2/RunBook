...
> The script's first menu offers a **choose-the-container-OS** decision: **debian** or **alpine** (one `vaultwarden.sh` now carries both — the separate `alpine-vaultwarden.sh` of older write-ups is gone). The alpine branch installs a prebuilt package into a smaller container — 256 MB RAM, a 1 GB disk — in seconds, no compiler involved. The trade: versions arrive on Alpine's packaging schedule rather than straight from the project, and its paths differ from everything this guide describes. Pick **debian**; the compiled build is the documented mainline here.
...
Add one layer the server can't take down with it: from the web vault, **Tools → Export**, format **.json (Encrypted)** — and switch the **Export type** radio from its **Account restricted** default (re-importable only into the *same* account — useless for disaster recovery) to **Password protected**, which reveals the **File password** field. Save the file onto the NAS, in with the irreplaceable files that the *Protect TrueNAS Data* guide's Cloud Sync task pushes offsite nightly — a vault whose only copies sit in one house isn't finished. Repeat after big additions; the export is a snapshot, not a feed.
