...
- **Three or four mains-powered Matter-over-Thread plugs are commissioned and placed**, forming a mesh that reaches every door. This is a prerequisite, not an upgrade — see the warning under *Add the first U400*. The Thread Group's own guidance is one router-capable device per **300–400 sq ft**, or three to four for a typical 1,500 sq ft home; a basement door needs its own, since floor assemblies eat 2.4 GHz. The **Nanoleaf Essentials Smart Outlet** (~$25) is the pick here — two independently controlled outlets each, so four devices give you eight sockets and four routers for about $100. The **Wemo Smart Plug Thread** (~$25) is equivalent. **Eve Energy** (~$40) adds real per-device power logging and a Thread mesh diagnostics view in its app, which is worth paying for only if you want to measure one specific device: rack-level draw already arrives free over **NUT** from the CyberPower UPS on the *UPS & Safe Shutdown* page. Three rules: insist on **Thread** — most cheap "Matter" plugs are Wi-Fi and do nothing here, and **TP-Link Tapo is the trap to watch**, since the whole Matter line (P125M, P400M) is 2.4 GHz Wi-Fi only despite the Matter badge; prefer **plugs over bulbs**, because a bulb stops routing the moment someone flips the wall switch, which is a miserable intermittent fault to chase; and buy **outlets rather than single plugs** where you can, since the extra socket costs nothing. Commission them **working outward from the ZBT-2**, so each new one is in range of the last.
