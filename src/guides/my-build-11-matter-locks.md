...
Use the wizard, which flashes the firmware *and* installs and configures the app for you. In Home Assistant, go to **Settings → Devices & services**, find the **Home Assistant Connect ZBT-2** entry, select its **cog wheel**, and choose **Use as Thread adapter**. It takes a few minutes.

> [!WARNING]
> **Confirm the serial before you start.** This writes Thread-only firmware, and that adapter can never run Zigbee again. Two identical ZBT-2s are plugged in, and flashing the **Zigbee** one destroys your coordinator — every plug, leak sensor, and the valve drop off and all of them need re-pairing from scratch. Check which serial the entry refers to against the `/dev/serial/by-id/usb-Nabu_Casa_ZBT-2_…` path already in **Settings → Apps → Zigbee2MQTT → Configuration**: that serial is the Zigbee radio and is the one you must **not** touch. If the entry does not make the serial plain, stop and identify it before clicking.
