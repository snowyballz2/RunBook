...
Do this on the **host**, not inside any container. From the Proxmox node shell, install the kernel headers and the NVIDIA driver, then confirm the card is seen:

```bash
# Headers matching the running Proxmox kernel
apt update
apt install -y pve-headers-$(uname -r) build-essential

# Install the official NVIDIA driver (download the .run from nvidia.com,
# or use your preferred packaging). After install, verify:
nvidia-smi
```
...
Without the persistence daemon the driver de-initialises whenever nothing is actively using the card, and the first detection after an idle stretch pays a slow wake-up cost. Enable it on the host:

```bash
systemctl enable --now nvidia-persistenced
```

### Lend the card into each LXC with dev0: syntax
Use Proxmox 8.1+'s `dev0:` device syntax rather than hand-writing `lxc.cgroup2` device lines — the newer syntax is what Proxmox officially supports and it survives upgrades. Edit each container's config file on the host (`/etc/pve/lxc/<ctid>.conf`) and add the NVIDIA device nodes. The same three lines go into the **Frigate**, **Ollama**, and **faster-whisper** containers:
...
Restart each container after editing its config. Inside the container, `nvidia-smi` should now show the same card.
...
Passthrough needs the card isolated in its own **IOMMU (Input/Output Memory Management Unit)** group. The HBA is in the bottom **x4_3 chipset-attached slot** (set to x4 in BIOS) precisely so it lands in a group by itself. Verify before binding anything:
...
You want the HBA in a group containing only itself (or only its own functions). If it shares a group with devices you care about, passthrough either fails or drags those neighbours into the VM — recheck that it's in the chipset-attached x4_3 slot.
...
```bash
# Equivalent from the host shell (replace 0000:01:00.0 with the HBA's address):
qm set <truenas-vmid> -hostpci0 0000:01:00.0,pcie=1
```
...
> - **TrueNAS sees no disks / `duplicate serial` errors** — the HBA didn't pass through cleanly. Confirm `Kernel driver in use: vfio-pci` on the host and that the device is added to the VM with **All Functions**.
> - **Passthrough fails or drags other devices in** — the HBA isn't alone in its IOMMU group. Confirm it's in the chipset-attached x4_3 slot at x4 and that VT-d is enabled in BIOS.
