...
With the host current, give each guest its turn — and go strictly one at a time: update it, confirm it still answers, then move to the next. A breakage always has an obvious author that way, and a snapshot to fall back to. Snapshot first whenever an update looks major.
