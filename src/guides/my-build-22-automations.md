...
The presence rules below reach for `climate.*` and `light.*` entities. The Caséta `light.*` entities already exist — you added the Lutron Caséta bridge back on the Home Assistant & Zigbee2MQTT page. The one integration still missing is the **ecobee thermostats**, so add it here: in **Settings → Devices & services → Add integration**, add **ecobee**. Its dialog still opens with an **API key** field — since Home Assistant 2026.3 **leave it blank**; no developer key is needed — then sign in with your ecobee **username** and **password** (a cloud integration, so the account must work). If the account has multi-factor enabled, a **6-digit authenticator code** prompt follows — authenticator-app codes are the only MFA kind the integration supports. Then note the two `climate.*` entity names it creates (the rules below use `climate.downstairs`; substitute your real names). The Caséta dimmers are already `light.*` entities from that earlier step, ready for the presence rules here and the scenes at the end of this page.
