...
> ```
>
> This is also how **Tailscale** and **NUT** stay current: both were installed on the host as `apt` packages (the *Remote Access* and *UPS Shutdown* guides), so the host upgrade carries them along — there's no separate step for either.
...
The free-space glance is its own habit, because a full disk fails loudly and at the worst time: check Frigate's footage disk and Nextcloud's storage (their own guides), and on the **Storage** dashboard in TrueNAS keep the ZFS pool under roughly **80%** — past that, ZFS slows down and snapshots have nowhere to grow. The backup storage on the NAS counts too; the *Proxmox Backups* retention settings are what keep it from creeping upward forever.

> [!TIP]
> While you're in **Datacenter → Backup**, open the job and confirm **AdGuard** and **NPM** are actually in the selection — All-mode includes them, but a hand-picked list is one careless edit away from dropping the two guests you can least afford to lose. AdGuard is the household's DNS, and NPM holds the reverse-proxy routes and certificates; restore everything *except* those two and the rest is unreachable until you rebuild them by hand.
