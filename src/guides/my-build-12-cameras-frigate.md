...
- **Maintenance Center → Update** → two halves on one page. Under **Online Update**: **Auto Check for Updates → off**, its shipped state — verify (the **Manual Check** button beside it is inert with the dead-end gateway; nothing reaches Dahua's servers either way). Under **File Update**: the real path — manual `.bin` files from EmpireTech's own download page, on your schedule
- **Maintenance Center → Log Info → Remote Log** → **Enable → off**, its shipped state — a syslog push of the camera's logs to a server; the prefilled `192.168.0.108` address is Dahua's placeholder, not a live destination. The rest of Maintenance Center (diagnosis, system info, local logs) and the **Storage** menu are local-only — out of this walk's scope
