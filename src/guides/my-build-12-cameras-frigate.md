...
- **The occupancy entity** — once the HA integration below is in: **Settings → Developer tools → States** (moved out of the sidebar in Home Assistant 2026.2; **Cmd/Ctrl+K** then `dev` is quicker), filter the camera; `binary_sensor.<camera>_person_occupancy` reads **on** while you stand in frame and drops to **off** when you leave — the whole camera → detector → MQTT → integration chain proven in one toggle
