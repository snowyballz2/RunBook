...
The installer walks a few text dialogs — accept the license, **No** to the 32-bit compatibility libraries, accept the rest as offered. Two blue **WARNING** dialogs appear, and both are expected. The first — it "will not install any kernel modules", matching modules must be "installed separately" — is `--no-kernel-module` doing its job: the kernel half lives on the host at exactly this version, from the GPU/HBA page. The second — it "was forced to guess the X library path" — is about X, the Linux desktop display system, which this headless container does not have and never needs; Frigate reaches the card through CUDA, not a screen. **OK** through both.
