...
5. **Everything else holds its default.** **Camera → Audio: leave disabled on both streams** — the turret config records no audio, outdoor voice capture is a legal and privacy line this build does not cross by default, and talk-back is the doorbell's job (enabling it someday means this toggle plus an audio record-preset in Frigate — the kitchen is the plausible candidate, as a household decision). **Encode → Overlay** is the one optional nicety: set the **Channel Title** to the camera's key and keep the **time overlay**, so exported clips self-identify in the pixels themselves. **Encode → ROI stays off** — that is region-of-interest *encoding* (bitrate favoritism inside the frame), not detection zones, which are Frigate's job later; uneven encoding just makes the unfavored part of an incident look worst. The **Alarm Tone** tab drives camera-side siren noises nothing here triggers.
