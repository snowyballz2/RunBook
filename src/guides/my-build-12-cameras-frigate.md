...
This build does **not** use the Intel iGPU + OpenVINO path that Frigate defaults to. Detection runs on the 1080 Ti via the **ONNX (Open Neural Network Exchange) detector on the CUDA (NVIDIA's GPU compute platform) execution provider** — this install's Frigate build ships the ONNX runtime, which picks up CUDA automatically once the card is visible, so pointing Frigate at ONNX is enough to find the card. Edit `/config/config.yml` — easiest in **Frigate's own web UI** at `https://192.168.1.52:8971`, whose built-in config editor validates as you type (`nano /config/config.yml` in the container's console works too) — and set:
...
2. **Deal with the unused pigtails before anything goes in the wall.** Running PoE leaves the 12 V barrel, alarm in/out, and audio leads doing nothing — **do not cut them off** (warranty aside, cutting opens a path into the cable jacket and forecloses any later use). Cap each unused end with **self-amalgamating tape** (bonds to itself, no residue, strips cleanly years later) or heat-shrink caps, with a dab of **dielectric grease** inside any connector shell first. An open connector is a wick: moisture corrodes the pins and can travel the conductors back into the camera body. Coil the capped leads, tuck them to the side of the cavity rather than against the drywall's back face where condensation forms, and zip-tie the bundle so nothing hangs on the camera's cable gland.
3. **Seal the outside.** Use the **base gasket** the camera shipped with — that is the actual seal; caulk is backup. Bead the base perimeter with **100% silicone or polyurethane** exterior sealant (never acrylic latex "painter's caulk", which chalks out in a season or two), and **leave the bottom arc open — roughly 5-to-7 o'clock, about an inch.** A fully sealed base traps the water that eventually gets in; the open bottom drains it instead of letting it pool behind the camera and soak the wall. Seal the **cable entry hole** too — the gap between hole and cable is the real entry point when water sheets down a wall — by packing it with backer rod or running a bead around the hole before mounting so the base compresses into it. Do **not** caulk the camera's own housing seams, lens ring, or vents: those carry factory gaskets and often a breathing membrane, and sealing them causes internal condensation rather than preventing it.
