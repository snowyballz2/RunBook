...
2. Seat its cooler.
3. Fit the 32 GB RAM.
4. Mount the Maximus X Hero in the View 71 on brass standoffs.
5. Install the Toughpower Grand RGB 850W in the bottom PSU shroud.
6. Run the 24-pin power now.
7. Run the 8-pin CPU power now.
8. Leave the PCIe power leads loose until the GPU is in.
...
2. Leave the two mirror drives' data connectors empty for now.
...
The **two mirror drives belong on the LSI/Broadcom 9300-8i HBA (host bus adapter)**, not the board — they follow the HBA into the TrueNAS VM (virtual machine). You cable them once the HBA is seated below.
...
- **CyberPower CP1500PFCLCD UPS** (uninterruptible power supply) — the server plugs into its battery side, and so do the Fios router, the Fios ONT, and the Caséta bridge (the rack switches stay on wall AC); its USB data cable goes to the host for NUT (Network UPS Tools) monitoring later.
...
1. On another computer, go to `asus.com`.
2. Click **Support**.
3. Search "Maximus X Hero".
4. Click **Driver & Utility**.
5. Click **BIOS & Firmware**.
6. Download the latest **BIOS** file.
7. Note the version number — you will confirm it after the flash.
8. Unzip the download.
9. Run the bundled **BIOSRenamer** utility (included in the same zip) once. Strictly, that rename is only required for the board's **USB BIOS FlashBack** button-recovery method — EZ Flash takes the file as downloaded — but renaming costs nothing and leaves the stick ready for either path.
10. Format a USB stick as **FAT32**.
11. Copy the renamed BIOS file to its **root** (not inside a folder).
12. Plug the stick into the server.
13. Enter the BIOS (`Del` on power-on).
14. Open **EZ Flash 3** under the *Tool* menu.
15. Point it at the file on the stick.
16. Confirm. The board flashes the firmware, reboots itself, and lands back in the BIOS on its own.
17. Confirm the BIOS version on the **EZ Mode** main screen (also listed in EZ Flash 3 itself) matches the version you downloaded, before continuing with the toggles below.
...
3. Find **Intel (VMX) Virtualization Technology** (older firmware labels it *Intel Virtualization Technology*).
4. Set it to **Enabled**. This is the core CPU virtualization switch that lets Proxmox run VMs at all.

### Enable Intel VT-d
1. Go to the *Advanced* tab.
2. Go to *System Agent (SA) Configuration* — a **different submenu** than VMX, so do not assume the first toggle covered it.
3. Find **VT-d** (Intel Virtualization Technology for Directed I/O).
4. Set it to **Enabled**. This is the switch that makes PCIe passthrough possible — without it, the 9300-8i can never be handed to the TrueNAS VM.
...
### Tune the fan curves
Press `F6` to open **Q-Fan Control** (the same screen lives at *Advanced → Monitor → Q-Fan Configuration*). This box idles 24/7 upstairs in lived-in space, far below the thermal load the default curves assume — quiet is not optional here:
...
2. **The optional top exhaust rides W_PUMP+ — deal with that header here.** The pump headers (W_PUMP+ and AIO_PUMP) ship at 100% because they expect a pump, not a fan. Set **W_PUMP+ Control** to **PWM Mode**.
3. Give it the **Silent** profile, or it runs flat out forever.
4. Leave **CPU_FAN** and **CPU_OPT** on **Standard**. The Phantom Spirit is quiet at idle and you want it free to ramp under load.
5. **Stick to the preset profiles** — skip the graphical curve editor and the Q-Fan Tuning sweep.
6. Press `F10` right after the fan settings so a hang can't take the rest of the session with it.
...
> [!TIP]
> The FTW3's own lights are set from EVGA's Windows software, not this BIOS — and Linux has no tool for them. While Windows still exists on the NVMe:
>
> 1. Open **Precision X1**.
> 2. Turn the GPU LEDs off.
>
> Settings made there generally persist on the card after the OS is gone. Two minutes now, or a GPU glowing through the glass in the room with you indefinitely.
...
3. Press `F10`.
4. Confirm. The board reboots on its own.
5. Leave the monitor and keyboard attached for now — you will need them for the OS install.
...
> 4. Click **CPU**.
> 5. Confirm the right-hand column reads **Virtualization: Enabled**.
>
