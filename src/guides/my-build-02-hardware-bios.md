...
- **Onboard Devices Configuration → LED lighting** — set the *working state* entry to **Aura Off** (or Stealth Mode) and the *sleep/soft-off* entry off too. If the firmware lists a separate **onboard LED** toggle for the logo and accent lighting, that goes off as well. A couple of watts, and a basement server has no audience. The one light worth its keep: the **Q-Code display** — it is the board's only voice when a headless box refuses to POST (it is how this build's own unseated-NVMe saga got diagnosed), so if a toggle names it specifically, leave that one alone.
