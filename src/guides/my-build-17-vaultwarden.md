...
At `https://vault.example.com`, the login screen asks for an email first — the signup door is the **Create account** link under **"New to Bitwarden?"**. It wants **Email** and **Name**, then the master password twice plus an optional hint; with no mail server configured there is no verification email — the account simply exists. Make yours, then one per family member. The master password is the one password each person must actually remember — make it a long passphrase.
