...
After the shutdown the UPS cuts its outlets, and when mains returns it switches them back on — but a powered outlet only boots the server if the firmware agrees. In the ASUS ROG Maximus X Hero BIOS (the same screens used during the hardware-and-BIOS prep), find **Advanced → APM Configuration → Restore AC Power Loss** and set it to **Power On**. NUT recommends "always power on" over "last state": the UPS shutdown was clean, so "last state" can remember *off* and leave the server dark in a powered house.
...
> [!TIP]
> Once, ever, on a quiet day, pull the plug and let the battery actually drain until the CyberPower itself declares `OB LB` and triggers everything. It is the only test that proves the low-battery trigger fires with margin to spare, at the cost of one battery cycle — worth doing once so the timed drill above has a real-world anchor.
