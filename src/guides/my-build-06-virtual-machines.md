...
2. TrueNAS offers **Test Changes** with a rollback timer. Click it, then **move to the new address promptly**: browse to `http://192.168.1.20`, log back in, and click **Save Changes** before the countdown expires — the timer is a safety net that reverts everything if you cannot reach the new address, which also means dawdling undoes the change. (If it does roll back, nothing is broken; edit and try again. The console's network option is the fallback of last resort.)
