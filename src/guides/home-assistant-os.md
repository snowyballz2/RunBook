...
>
> For the radio, pass the **Home Assistant Connect ZBT-2** coordinator through to wherever Z2M runs (the USB passthrough from above) and select the **`ember`** driver in Z2M's settings — that's the one for this coordinator. With the broker connected and the radio adopted, put each device into pairing mode and join it — leak sensors, smart plugs, a water shut-off valve, whatever you are adding. Once paired, Z2M reports them over MQTT and Home Assistant's MQTT integration surfaces them as ordinary entities, like a `binary_sensor.*` for a leak sensor or a `valve.*` for a smart valve.
>
> That last part is the point: if you build the *Automations* guide's water-leak → valve automation, this onboarding is its prerequisite. Until leak sensors and a valve exist as Home Assistant (HA) entities, there's nothing for that automation to listen to or close.
...
> If you use Apple Home, a natural first move is to commission the lock straight into **Apple Home** — that's what unlocks Home Key, and an Apple Thread border router (a HomePod or Apple TV) carries the Thread network. The catch: that single-use QR code is now **spent**, so Home Assistant can't scan it again. Matter is built for this, though — a device can answer to several controllers at once (*multi-admin*), so you **share** the lock instead of re-pairing it.
>
