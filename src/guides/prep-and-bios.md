...
>
> - **CPU** — anything 64-bit with virtualization (Intel VT-x (Virtual Machine Extensions) or AMD-V). Practically every desktop CPU since around 2010 has it; it just may be switched off in the BIOS (next steps).
> - **RAM** — Proxmox itself wants about 2 GB; every container and VM (virtual machine) you run needs its own share. 8 GB is a workable start, 16 GB+ is comfortable.
