...
>
> **One deliberate exception, and it matters: the Home Assistant VM.** Matter and Thread are **IPv6-only protocols**, so the locks, the Thread plugs and the door sensors are all reached over IPv6. Turning IPv6 off at the router does not touch that — Thread devices talk over link-local (`fe80::`) plus an `fd…` prefix the border router advertises itself, neither of which needs anything from Verizon. What *would* break it is carrying the IPv4-only habit onto Home Assistant's own interface: leave **Settings → System → Network → IPv6** on **Automatic** on that VM, and read a stray `fd…` address there as the Thread mesh working, not as something to clean up. Everything else — every LXC, TrueNAS, the router itself — stays IPv4-only exactly as written.
>
...
> [!WARNING]
> **A secondary resolver breaks local names, intermittently.** The next page has AdGuard answering `*.kuzco.org → 192.168.1.54`, and those rewrites exist nowhere else — a query that lands on a public secondary returns NXDOMAIN for them. So `frigate.kuzco.org` resolves, then does not, then does, depending on which resolver a client happened to ask. That intermittency is genuinely miserable to diagnose because every symptom points at the proxy or the certificate instead of at this setting. Local hostnames and DHCP device names fail the same way.
>
